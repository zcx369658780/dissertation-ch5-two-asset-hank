"""Prepare and run exactly one corrected-2018 two-turn trajectory."""
...
VERDICT_PASS = "CORRECTED_2018_TWO_TURN_PASS__TURN2_PROPAGATION_OBSERVED"
VERDICT_MISMATCH = "CORRECTED_2018_TWO_TURN_FAIL__TURN1_REPRODUCTION_MISMATCH"
VERDICT_HOUSEHOLD = "CORRECTED_2018_TWO_TURN_FAIL__HOUSEHOLD_OR_NUMERICAL_BLOCKER"
VERDICT_UPSTREAM = "CORRECTED_2018_TWO_TURN_FAIL__UPSTREAM_STATE_OR_FIRM_BLOCKER"
ACCEPTED_DIR = REPO / "reports/mp4c_2018_corrected_single_turn_20260909"
ACCEPTED_SHA256 = {
    "per_province_observables": "FF477D2F6C65573C153936A29BD6FD17195418F9C01AA1B9EC79903676CBEDE9",
    "controller_observables": "6B3153FF8C5EDEFEB547CA87089092E6D6C980B0EDC16034771C15FFB64C554C",
    "manifest": "23B8C31417CC0AA9715D8AD0D0F92D835C7344143D3832392BA7F883A81D369C",
}
...
def prepare(distance_workbook: Path, evidence_root: Path) -> None:
    ...
    payload["schema"] = "CH5_CORRECTED_2018_TWO_TURN_RUNTIME_INPUT_V1"
    payload_path = root / "runtime_input_payload.json"
    write_json(payload_path, payload)
    accepted_paths = {
        "per_province_observables": ACCEPTED_DIR / "per_province_observables.json",
        "controller_observables": ACCEPTED_DIR / "controller_observables.json",
    ...
    write_json(root / "runtime_input_receipt.json", {
        "schema": "CH5_CORRECTED_2018_TWO_TURN_RUNTIME_RECEIPT_V1",
        ...
        "grid": {"I": 20, "b": [-2.0, 5.0], "J": 20, "a": [0.0, 10.0],
                 "Nz": 2, "z": [0.8, 1.3]},
        "accepted_turn1": {name: {"path": str(path.relative_to(REPO)).replace("\\", "/"),
                                    "sha256": accepted_actual[name]}
                           for name, path in accepted_paths.items()},
        "scientific_calls": 0,
    })
    write_json(root / "source_code_identity.json", {
        "schema": "CH5_CORRECTED_2018_TWO_TURN_SOURCE_IDENTITY_V1",
...
def compare_fresh_turn1(rows: list[dict[str, Any]], controller: dict[str, Any]) -> dict[str, Any]:
    accepted_rows = json.loads((ACCEPTED_DIR / "per_province_observables.json").read_text(encoding="utf-8"))
    accepted_controller = json.loads((ACCEPTED_DIR / "controller_observables.json").read_text(encoding="utf-8"))
    ...
    for key, expected in accepted_controller.items():
        actual = controller.get(key, "__MISSING__")
        if actual != expected:
            mismatches.append({"path": f"controller.{key}", "fresh": actual, "accepted": expected})
    return {
        "schema": "CH5_CORRECTED_2018_FRESH_TURN1_REPRODUCTION_V1",
        "comparison_rule": "EXACT_JSON_VALUE_AND_SOURCE_ORDER_IDENTITY",
        "accepted_rows_sha256": single.file_sha256(ACCEPTED_DIR / "per_province_observables.json"),
...
def rah_provenance(turn: int, index: int, states: tuple[Mapping[str, object], ...],
    ...
    if prior_states is None:
        raise ValueError("turn-2 rah provenance requires turn-1 entering states")
    ...
    return {
        ...
        "source": "turn_1.one_turn.capital.household_illiquid_return_rah -> steady_state._post_turn_states.rah",
        "formula": "(1-ratio_i)*old_ra_i + ratio_i*(sum_j(ratio_j*old_ra_j)-ratio_i*old_ra_i)/(n-1)",
        "source_old_ra_turn": 1, "source_old_ra_field": "turn_1.entering_state.ra",
...
def execute(evidence_root: Path) -> int:
    ...
    counters = {key: 0 for key in (
        ...
        "wage_batch_calls", "wage_province_outputs", "controller_calls", "adaptation_blocks")}
    counters.update({"scientific_processes": 1, "trajectory_calls": 1, "scientific_retries": 0})
    write_json(root / "science_started.json", {
        "schema": "CH5_CORRECTED_2018_TWO_TURN_LAUNCH_V1",
        ...
        "authorized_trajectory_count": 1, "authorized_turns": 2, "scientific_retries": 0,
        "runtime_payload_sha256": single.file_sha256(payload_path),
    })
    ...
    try:
        for turn_index in (1, 2):
            ...
            if turn_index == 1:
                comparison_rows = [{key: row[key] for key in json.loads(
                    (ACCEPTED_DIR / "per_province_observables.json").read_text(encoding="utf-8"))[index]}
                                   for index, row in enumerate(rows)]
                reproduction = compare_fresh_turn1(comparison_rows, controller)
                write_json(root / "turn1_reproduction.json", reproduction)
    ...
    counters["household_calls_failed"] = counters["household_calls_attempted"] - counters["household_calls_returned"]
    write_json(root / "transition_summary.json", {"schema": "CH5_CORRECTED_2018_TWO_TURN_TRANSITION_V1",
        "turns": turn_summaries, "turn2_completed": counters["turns_completed"] == 2})
    write_json(root / "call_ledger.json", {"schema": "CH5_CORRECTED_2018_TWO_TURN_CALL_LEDGER_V1",
        "counts": counters, "elapsed_seconds": time.monotonic() - started, "failed_calls_counted": True,
        "matlab_calls": 0, "third_turns": 0, "steady_state_calls": 0, "ge_calls": 0,
        "annual_model_calls": 0, "irf_calls": 0, "results_calls": 0})
    write_json(root / "terminal_result.json", {"schema": "CH5_CORRECTED_2018_TWO_TURN_TERMINAL_V1",
        "verdict": verdict, "error": error, "third_turn_authorized": "NO",
