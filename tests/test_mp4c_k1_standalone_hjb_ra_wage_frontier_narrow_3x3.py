...
from validators.multi_province.k1_standalone_hjb_ra_wage_frontier_narrow_3x3 import (
    finalize,
    run,
)


def test_exact_narrow_grid_and_frozen_configuration() -> None:
    assert run.POINTS == tuple(
        (ra, wage)
        for ra in (0.06, 0.0675, 0.07)
        for wage in (0.8, 1.05, 1.3)
    )
    fixture = run.build_fixture(0.0675, 1.05)
    ...
    assert fixture.inputs.r_a == 0.0675
...
def test_outside_narrow_grid_is_rejected() -> None:
    with pytest.raises(ValueError, match="outside the preregistered narrow grid"):
        run.build_fixture(0.065, 1.05)


def test_only_ra_and_wage_vary_and_initial_arrays_are_fresh() -> None:
    left = run.build_fixture(0.06, 0.8)
    same = run.build_fixture(0.06, 0.8)
    right = run.build_fixture(0.07, 1.3)
...
def test_next_gate_stops_scalar_refinement_when_no_universal_ra_exists() -> None:
    assert finalize.recommended_next_gate([])["name"] == (
        "OWNER_REVIEW_WAGE_CONDITIONAL_RA_W_HEALTH_REGION_AND_PROVINCIAL_RETURN_MAPPING"
    )
    assert finalize.recommended_next_gate([])["run_now"] is False
    assert finalize.recommended_next_gate([0.0675])["name"] == (
        "OWNER_REVIEW_PROVISIONAL_WAGE_ROBUST_RA_HEALTH_BAND_FREEZE"
    )
