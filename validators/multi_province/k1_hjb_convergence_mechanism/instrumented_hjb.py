"""Observation-only copy of the accepted HJB map with per-iteration diagnostics.

The scientific statements and their order mirror
``exports.matlab_faithful_two_asset_ha.solve_matlab_faithful_hjb``.  Diagnostic
operations only read values after the accepted scientific operations complete.
"""
...
def _summary(value: np.ndarray) -> dict[str, Any]:
    ...
    if finite.size:
        ...
        result.update({
            "min": float(quantiles[0]), "p01": float(quantiles[1]),
            "median": float(quantiles[2]), "p99": float(quantiles[3]),
            "max": float(quantiles[4]),
        })
    return result


def _operator_metrics(operator: sparse.csr_matrix) -> dict[str, Any]:
    matrix = operator.tocsr()
    row_sum = np.asarray(matrix.sum(axis=1)).ravel()
    coo = matrix.tocoo()
    off = coo.data[coo.row != coo.col]
    dense = matrix.toarray()
    header = f"{dense.dtype.str}|{dense.shape}|C|".encode("ascii")
    return {
        "max_abs_row_sum_defect": float(np.max(np.abs(row_sum))),
        "minimum_stored_off_diagonal": float(np.min(off)) if off.size else 0.0,
        "full_sha256": sha256(header + np.ascontiguousarray(dense).tobytes()).hexdigest().upper(),
    }


def solve_with_observations(
    grid: Any,
    params: Any,
    inputs: Any,
    ...
    """Run the accepted HJB map while returning read-only diagnostic summaries."""
    ...
    for iteration in range(1, numerics.max_iterations + 1):
        ...
        # Everything below is observation-only and is never read by scientific control flow.
        update = value - old
        flat_index = int(np.argmax(np.abs(update)))
        argmax = tuple(int(x) for x in np.unravel_index(flat_index, shape))
        one_step = float(np.max(np.abs(update)))
        two_step = (float(np.max(np.abs(value - two_back_value)))
                    if two_back_value is not None else None)
        ...
        used_vb_f = np.maximum(vb_f, floor); used_vb_b = np.maximum(vb_b, floor)
        residual = matrix @ value.ravel(order="F") - rhs
        observations.append({
            "iteration": iteration,
            "convergence_statistic": statistic,
            "value_update_inf": one_step,
            "value_update_argmax_zero_based": list(argmax),
            "value_update_signed_at_argmax": float(update[argmax]),
            "value_two_step_inf": two_step,
            "two_step_to_one_step_ratio": (two_step / one_step if two_step is not None and one_step != 0.0 else None),
            ...
            "derivatives": {
                ...
                "floor_hit_counts": {
                    ...
                    "vb_backward": int(np.count_nonzero(vb_b < floor)),
                    "va_forward": 0, "va_backward": 0,
                },
                "floor_hit_shares": {
                    "vb_forward": float(np.count_nonzero(vb_f < floor) / vb_f.size),
                    "vb_backward": float(np.count_nonzero(vb_b < floor) / vb_b.size),
                    "va_forward": 0.0, "va_backward": 0.0,
                },
                ...
                "max_raw_to_used_deviation": {
                    "vb_forward": float(np.max(np.abs(used_vb_f - vb_f))),
                    "vb_backward": float(np.max(np.abs(used_vb_b - vb_b))),
                    "va_forward": 0.0, "va_backward": 0.0,
                },
            },
            "selected": {name: _summary(arrays[name]) for name in (
    ...
    return result, {"schema": "CH5_MP4C_K1_HJB_MECHANISM_TRACE_V1", "iterations": observations}
