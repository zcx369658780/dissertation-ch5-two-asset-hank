"""Execute the preregistered real composite-wage standalone 3x3 scan."""
...
from validators.multi_province.k1_standalone_hjb_ra_wage_frontier_narrow_3x3.run import (
    illiquid_transition_receipt,
)


REPO = Path(__file__).resolve().parents[3]
POINTS = tuple((ra, wage) for ra in (0.06, 0.0675, 0.07) for wage in (13.0, 15.5, 18.0))
...
def build_fixture(ra: float, wage: float) -> coarse.Fixture:
    if (ra, wage) not in POINTS:
        raise ValueError("parameter point is outside the preregistered real composite-wage grid")
    grid = oracle.MatlabFaithfulHJBGrid(
        np.linspace(-2.0, 5.0, 20), np.linspace(0.0, 10.0, 20), np.array([0.8, 1.3]),
    ...
    inputs = oracle.HouseholdInputs(ra, 0.02, 0.05, np.array([wage]), np.zeros(1), np.ones(1))
...
def execute(evidence_root: Path) -> int:
    ...
    source_receipt = {
        "oracle": {"path": ORACLE_PATH, "expected_sha256": ORACLE_SHA256,
                   "actual_sha256": coarse.file_sha256(ORACLE_PATH)},
        "matlab": {"path": MATLAB_PATH, "expected_sha256": MATLAB_SHA256,
    ...
    if (invariance["unexpected_varying_fields"] or not invariance["all_non_scanned_fields_identical"]
    ...
    ledger = {
        "schema": "CH5_MP4C_K1_REAL_COMPOSITE_WAGE_3X3_LEDGER_V1",
        ...
        "matlab_runtime_calls": 0, "k1b_calls": 0, "k2_calls": 0, "ge_calls": 0,
    ...
    for index, ((ra, wage), fixture) in enumerate(zip(POINTS, fixtures), start=1):
        point_id = f"p{index:02d}_ra_{ra:.4f}_w_{wage:.1f}".replace(".", "p")
        ...
        point = {"point_index": index, "point_id": point_id, "r_b": 0.02, "r_a": ra,
                 "wage": wage, "hjb": hjb_receipt, "kfe": {"ran": False},
        ...
        if coarse.kfe_authorized(hjb_receipt["classification"]):
            ...
            try:
                ...
                np.savez_compressed(root / f"{point_id}_scientific_arrays.npz",
                                    value=hjb.value, consumption=hjb.consumption,
                                    labor=hjb.labor, transfer=hjb.transfer,
                                    mu_a=hjb.mu_a, mu_b=hjb.mu_b, density=kfe.density)
            except Exception as exc:
                point["kfe"] = {"ran": True, "completed": False,
                                "hard_error": f"{type(exc).__name__}: {exc}"}
        coarse.write_json(root / f"{point_id}_result.json", point)
        ledger["points"].append({"point_index": index, "point_id": point_id,
    ...
    coarse.write_json(root / "execution_complete.json", {
        "status": "EXACT_REAL_COMPOSITE_WAGE_3X3_EXECUTION_COMPLETE",
        "results_eligibility": False, "hjb_calls": ledger["hjb_calls_started"],
