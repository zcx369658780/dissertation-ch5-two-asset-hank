"""Solver-free finalization for complete or early-failed 25-turn K/L evidence."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    national_fields = [
        ...
        "hjb_failures", "kfe_failures", "kfe_diagnostic_only_count",
    ]
    national_rows = []
    ...
    write_csv(output / "scientific_validity_ledger.csv", validity,
              ["turn", "province_index", "province_name", "hjb_converged", "hjb_iterations",
               "hjb_convergence_statistic", "kfe_reached", "kfe_status",
               "kfe_source_free_normwise_ratio", "kfe_upper_b_max_outward_leak_rate"])

    status = "AVAILABLE" if late else "NOT_AVAILABLE_EARLY_HARD_FAILURE_BEFORE_LATE_WINDOW"
    ...
    write_json(output / "tests_static_check_receipt.json", {
        "schema": "CH5_CORRECTED_2018_25TURN_KL_STATIC_CHECKS_V1",
    ...
    write_json(output / "source_hash_receipt.json", {
        "schema": "CH5_CORRECTED_2018_25TURN_KL_SOURCE_HASH_RECEIPT_V1",
    ...
    manifest = {"schema": "CH5_CORRECTED_2018_25TURN_KL_MANIFEST_V1", "files": [
    ...
    write_json(output / "manifest_readback.json", {
        "schema": "CH5_CORRECTED_2018_25TURN_KL_MANIFEST_READBACK_V1",
