...
def _seal_manifest(evidence: Path) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "CH5_MP4C_2018_KFE_D123_TINY_REAL_CELL_SELECTOR_PANEL_MANIFEST_V1",
        "entries": entries,
...
def execute(repository: Path) -> int:
    ...
    for ordinal, cell in enumerate(panel, start=1):
        try:
            ...
            _write_json(
                ...
                _ledger(
                    budget,
                    ordinal - 1,
                    "STOPPED_AFTER_FIRST_REAL_CALL__NO_PATCH_OR_RETRY_AUTHORIZED",
                ),
            )
        ...
        _write_json(evidence / f"cell_{ordinal:02d}_{cell.panel_id}.json", receipt)
        _write_json(
