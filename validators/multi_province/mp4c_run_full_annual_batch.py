...
def valid_success(year_dir:Path,year:int,canonical_sha:str)->bool:
    marker=year_dir/"SUCCESS.json"
    if not marker.is_file(): return False
    try: p=json.loads(marker.read_text(encoding="utf-8"))
    except Exception:return False
    if p.get("schema")!="CH5_MP4C_YEAR_SUCCESS_V2" or p.get("representation")!=runtime_cache.REPRESENTATION or p.get("year")!=year or p.get("status")!="SOURCE_CONVERGED" or p.get("runtime_input_sha256")!=canonical_sha or p.get("runtime_cache_sha256")!=runtime_cache.EXPECTED_CACHE_SHA256:return False
...
def prepare_runtime_inputs(data_root:Path,cache_path:Path,batch_root:Path)->dict[int,Path]:
    result={}
    for year in YEARS:
        target=batch_root/"runtime_inputs"/str(year);path=target/f"calendar_{year}_matlab_runtime_cache_input.json"
        expected=runtime_cache.add_runtime_support(runtime_cache.load_runtime_year(cache_path,year),distance_workbook=empirical.primary_sources(data_root).distance_workbook,distance_sha256=empirical.SOURCE_HASHES["中国各省省会地理距离矩阵.xlsx"],max_sigmau=empirical.accepted_source_scalars().max_sigmau)
        expected_bytes=runtime_cache.canonical_bytes(expected)
        if target.exists():
...
def aggregate(root:Path,statuses:list[dict[str,Any]])->None:
    ...
    with (root/"steady_state_panel_2009_2023.csv").open("x",encoding="utf-8-sig",newline="") as f:
    ...
    for field in legacy:
        ...
        for name in names:
            ...
            ws.append([name,*vals])
    wb.save(root/"2009_2023_稳态值.xlsx")
    wb=Workbook();wb.remove(wb.active)
    for year in passed:
    ...
    wb.save(root/"2009_2023_稳态Ltmat.xlsx")

def run_batch(data_root:Path,cache_path:Path,root:Path,workers:int)->int:
    ...
    manifest={"schema":"CH5_MP4C_FULL_BATCH_V2","representation":runtime_cache.REPRESENTATION,"years":list(YEARS),"workers":workers,"data_root":str(data_root.resolve()),"runtime_cache_path":str(cache_path.resolve()),"runtime_cache_sha256":runtime_cache.EXPECTED_CACHE_SHA256,"output_root":str(root.resolve()),"python":sys.executable,"thread_environment":production.THREAD_ENV,"start":now()}
    manifest_path=root/"batch_manifest.json"
    if manifest_path.exists():
        existing=json.loads(manifest_path.read_text(encoding="utf-8"))
        if existing.get("schema")!="CH5_MP4C_FULL_BATCH_V2" or existing.get("years")!=list(YEARS) or existing.get("data_root")!=str(data_root.resolve()) or existing.get("runtime_cache_sha256")!=runtime_cache.EXPECTED_CACHE_SHA256: raise RuntimeError("shared batch manifest defect")
        manifest=existing
    else: production.write_json_no_overwrite(manifest_path,manifest)
    canon=prepare_runtime_inputs(data_root,cache_path,root); preflight_end=now(); statuses=[]; pending=[]
    for y in YEARS:
        yd=root/f"year_{y}"; csha=sha(canon[y])
        if yd.exists():
            if valid_success(yd,y,csha): statuses.append({"year":y,"status":"SKIP","queued":None,"start":None,"end":now(),"wall_clock_seconds":0,"exit_code":0})
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        while pending or active:
            ...
            print(f"[{len(statuses):02d}/15] running={running} PASS={passed} FAIL={failed} elapsed={time.monotonic()-overall:.0f}s",flush=True)
    aggregate(root,statuses)
    ...
    required=[root/n for n in ("batch_manifest.json","batch_summary.json","batch_summary.csv","batch_timing.json","steady_state_panel_2009_2023.csv","2009_2023_稳态值.xlsx","2009_2023_稳态Ltmat.xlsx","matlab_persistence_contract.json")]
...
def main(argv=None)->int:
    p=argparse.ArgumentParser();p.add_argument("--data-root",required=True);p.add_argument("--runtime-cache",required=True);p.add_argument("--output-root",required=True);p.add_argument("--workers",type=int,default=DEFAULT_WORKERS);a=p.parse_args(argv)
    return run_batch(Path(a.data_root),Path(a.runtime_cache),Path(a.output_root),a.workers)
