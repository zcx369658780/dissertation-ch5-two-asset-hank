"""Execute the exact Owner-approved standalone ra-transition 3x3 scan."""
...
POINTS = tuple((ra, wage) for ra in (0.065, 0.0725, 0.08) for wage in (0.8, 1.05, 1.3))
...
def build_fixture(ra: float, wage: float) -> coarse.Fixture:
    if (ra, wage) not in POINTS:
        raise ValueError("parameter point is outside the preregistered refinement grid")
...
def execute(evidence_root: Path) -> int:
    ...
    ledger = {
        "schema": "CH5_MP4C_K1_STANDALONE_HJB_RA_TRANSITION_REFINEMENT_3X3_LEDGER_V1",
    ...
    coarse.write_json(root / "execution_complete.json", {
        "status": "EXACT_REFINEMENT_3X3_EXECUTION_COMPLETE",
