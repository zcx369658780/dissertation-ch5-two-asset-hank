"""Propagate finite HJB nonconvergence and run one bounded corrected-2018 trajectory."""
...
VERDICT_PASS = "HJB_PROPAGATION_REPAIR_AND_25TURN_KL_PASS__LATE_WINDOW_RECONCILIATION_COMPLETE"
VERDICT_PARTIAL = "HJB_PROPAGATION_REPAIR_PASS__25TURN_KL_PARTIAL_TRUE_HARD_FAILURE_BEFORE_LATE_WINDOW"
VERDICT_BLOCKED = "HJB_PROPAGATION_REPAIR_BLOCKED__SOURCE_STYLE_CONTINUATION_NOT_IMPLEMENTABLE_WITH_CURRENT_RETURN_OBJECTS"
THREAD_ENV = ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "NUMEXPR_NUM_THREADS")
EXPECTED_HJB_SHAPE = (20, 20, 2)
EXPECTED_HJB_SIZE = int(np.prod(EXPECTED_HJB_SHAPE))
HJB_NUMERIC_ARRAY_FIELDS = (
...
def _require_finite_array(owner: Any, field: str, shape: tuple[int, ...]) -> np.ndarray:
    if owner is None or not hasattr(owner, field):
        raise ValueError(f"unusable return object: missing {field}")
    value = np.asarray(getattr(owner, field))
    if value.shape != shape:
        raise ValueError(f"malformed {field} shape: {value.shape}, expected {shape}")
    if not np.issubdtype(value.dtype, np.number) or not np.isfinite(value).all():
        raise ValueError(f"nonfinite or nonnumeric required array: {field}")
    return value


def _require_finite_sparse_operator(owner: Any, field: str) -> Any:
    operator = getattr(owner, field, None)
    full = getattr(operator, "full", None)
    if full is None or not hasattr(full, "tocsr"):
        raise ValueError(f"unusable HJB operator: {field}")
    matrix = full.tocsr()
    if matrix.shape != (EXPECTED_HJB_SIZE, EXPECTED_HJB_SIZE):
        raise ValueError(f"malformed HJB operator shape: {field}={matrix.shape}")
    if not np.isfinite(matrix.data).all():
        raise ValueError(f"nonfinite HJB operator: {field}")
    return matrix


def validate_hjb_return_for_continuation(hjb: Any) -> str:
    """Fail closed on unusable state, but classify finite nonconvergence for continuation."""
    if hjb is None:
        raise ValueError("no usable HJB return object")
    for field in HJB_NUMERIC_ARRAY_FIELDS:
        _require_finite_array(hjb, field, EXPECTED_HJB_SHAPE)
    _require_finite_sparse_operator(hjb, "operator")
    _require_finite_sparse_operator(hjb, "post_convergence_operator")
    statistic = float(getattr(hjb, "convergence_statistic", float("nan")))
    iterations = getattr(hjb, "iterations", None)
    if not np.isfinite(statistic):
        raise ValueError("nonfinite HJB convergence statistic")
    if not isinstance(iterations, (int, np.integer)) or int(iterations) < 1:
        raise ValueError("malformed HJB iteration count")
    return "HJB_CONVERGED" if bool(getattr(hjb, "converged", False)) else "HJB_NONCONVERGED_DIAGNOSTIC_ONLY"


def validate_kfe_return_for_continuation(kfe: Any) -> None:
    if kfe is None:
        raise ValueError("no usable KFE return object")
    _require_finite_array(kfe, "density", EXPECTED_HJB_SHAPE)
    for field in ("density_vector", "rhs", "raw_solve_vector"):
        _require_finite_array(kfe, field, (EXPECTED_HJB_SIZE,))
    for field in ("original_operator", "transpose", "contaminated_matrix"):
        matrix = getattr(kfe, field, None)
        if matrix is None or not hasattr(matrix, "tocsr"):
            raise ValueError(f"unusable KFE operator: {field}")
        matrix = matrix.tocsr()
        if matrix.shape != (EXPECTED_HJB_SIZE, EXPECTED_HJB_SIZE) or not np.isfinite(matrix.data).all():
            raise ValueError(f"malformed or nonfinite KFE operator: {field}")
    for field in ("normalization_factor", "db", "da", "cell_weight", "raw_residual_inf"):
        if not np.isfinite(float(getattr(kfe, field, float("nan")))):
            raise ValueError(f"nonfinite KFE scalar: {field}")


def build_household_batch(outputs: list[dict[str, Any]], expected_count: int = 31) -> PreFrozenHouseholdOutputBatch:
    if len(outputs) != expected_count:
        raise ValueError(f"household batch count {len(outputs)} != {expected_count}")
    batch = PreFrozenHouseholdOutputBatch(
        ...
        at_tax=[row["AtTax"] for row in outputs], converged=tuple(bool(row["hjb_converged"]) for row in outputs),
        diagnostics=tuple({"hjb_converged": bool(row["hjb_converged"]),
            "hjb_iterations": row["hjb_iterations"], "hjb_statistic": row["hjb_statistic"],
            "hjb_acceptance_classification": row["hjb_acceptance_classification"],
            "downstream_kfe_attempted": row["downstream_kfe_attempted"],
            "downstream_kfe_returned": row["downstream_kfe_returned"],
            "continuation_status": row["continuation_status"]} for row in outputs))
    return batch


def all_household_hjb_converged(batch: PreFrozenHouseholdOutputBatch) -> bool:
    """The source final predicate may pass only when every household flag is true."""
    return len(batch.converged) == 31 and all(bool(value) for value in batch.converged)


def record_phase_a(evidence_root: Path, test_processes: int, test_cases: int, compile_processes: int) -> None:
    root = Path(evidence_root)
    root.mkdir(parents=True, exist_ok=False)
    write_json(root / "phase_a_zero_science_repair_receipt.json", {
        "schema": "CH5_HJB_PROPAGATION_PHASE_A_ZERO_SCIENCE_REPAIR_V1",
        "status": "PASS",
        "focused_test_processes": test_processes,
        "focused_test_cases": test_cases,
        "compile_processes": compile_processes,
        "hjb_calls": 0, "kfe_calls": 0, "outer_turn_calls": 0, "scientific_calls": 0,
        "repair_contract": {
            "finite_nonconverged_hjb_continues": True,
            "false_flag_preserved": True,
            "false_flag_blocks_final_source_predicate": True,
            "nonfinite_or_malformed_return_fails_closed": True,
        },
    })


def _require_phase_a_receipt(root: Path) -> dict[str, Any]:
    path = root / "phase_a_zero_science_repair_receipt.json"
    if not path.is_file():
        raise ValueError("Phase A zero-science repair receipt is missing")
    receipt = json.loads(path.read_text(encoding="utf-8"))
    if receipt.get("status") != "PASS" or receipt.get("scientific_calls") != 0:
        raise ValueError("Phase A zero-science repair gate did not pass")
    if any(receipt.get(key) != 0 for key in ("hjb_calls", "kfe_calls", "outer_turn_calls")):
        raise ValueError("Phase A receipt records a scientific call")
    return receipt


def prepare(distance_workbook: Path, evidence_root: Path) -> None:
    root = Path(evidence_root)
    _require_phase_a_receipt(root)
    ...
    payload["schema"] = "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_RUNTIME_INPUT_V1"
    payload_path = root / "runtime_input_payload.json"
    write_json(payload_path, payload)
    write_json(root / "runtime_input_receipt.json", {
        "schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_RUNTIME_RECEIPT_V1",
    ...
    write_json(root / "source_code_identity.json", {
        "schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_SOURCE_IDENTITY_V1",
    ...
    write_json(root / "bounded_invocation_receipt.json", {
        "schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_BOUNDED_INVOCATION_V1",
        "prepare_entrypoint": "validators/multi_province/corrected_2018_hjb_propagation_25turn_kl/run.py prepare",
        "science_entrypoint": "validators/multi_province/corrected_2018_hjb_propagation_25turn_kl/run.py run",
...
def national_summary(turn: int, rows: list[dict[str, Any]], controller: dict[str, Any]) -> dict[str, Any]:
    ...
    hjb_nonconverged = [row for row in rows if not row["hjb_converged"]]
    result = {
        "turn": turn,
        ...
        "hjb_converged_count": len(rows) - len(hjb_nonconverged),
        "hjb_nonconverged_but_continued_count": len(hjb_nonconverged),
        "hjb_nonconverged_provinces": [row["province"] for row in hjb_nonconverged],
        "hjb_nonconverged_details": [{"province": row["province"],
            "iterations": row["hjb_iterations"], "statistic": row["hjb_statistic"]}
            for row in hjb_nonconverged],
        "hjb_failures": len(hjb_nonconverged),
...
def execute(evidence_root: Path) -> int:
    ...
    write_json(root / "science_started.json", {
        "schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_LAUNCH_V1",
    ...
    try:
        for turn_index in range(1, 26):
            ...
            for index, state in enumerate(states):
                ...
                hjb_classification = validate_hjb_return_for_continuation(hjb)
                ...
                write_json(province_root / "hjb_receipt.json", {"turn": turn_index, "province_index": index,
                    ...
                    "hjb_acceptance_classification": hjb_classification,
                    "persisted_before_kfe": True, "downstream_kfe_attempted": True,
                    "continuation_reason": (
                        "FINITE_STRUCTURALLY_USABLE_HJB_RETURN"
                        if hjb.converged else "FINITE_NONCONVERGED_HJB_RETURN_PROPAGATED_DIAGNOSTIC_ONLY")})
                ...
                validate_kfe_return_for_continuation(kfe)
                ...
                output = {"hjb_converged": hjb.converged, "hjb_iterations": hjb.iterations,
                    "hjb_statistic": hjb.convergence_statistic,
                    "hjb_acceptance_classification": hjb_classification,
            ...
            batch = build_household_batch(outputs)
            ...
            every_household_converged = all_household_hjb_converged(batch)
            if converged and not every_household_converged:
                raise RuntimeError("source final predicate passed despite a false household HJB flag")
            ...
            controller = {"household_converged_count": household_count,
                "all_household_hjb_converged": every_household_converged,
            ...
            for index, state in enumerate(states):
                ...
                row = {"turn": turn_index, "province_index": index, "province": state["name"],
                    ...
                    "hjb_statistic": output["hjb_statistic"],
                    "hjb_acceptance_classification": output["hjb_acceptance_classification"],
                    "downstream_kfe_attempted": output["downstream_kfe_attempted"],
                    "downstream_kfe_returned": output["downstream_kfe_returned"],
                    "continuation_status": output["continuation_status"], "kfe_returned": True,
    ...
    counters["household_calls_failed"] = counters["household_calls_attempted"] - counters["household_calls_returned"]
    write_json(root / "transition_summary.json", {"schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_TRANSITION_V1",
    ...
    write_json(root / "call_ledger.json", {"schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_CALL_LEDGER_V1",
    ...
    write_json(root / "terminal_result.json", {"schema": "CH5_HJB_PROPAGATION_CORRECTED_2018_25TURN_KL_TERMINAL_V1",
